...
/*   let returnType = usersPr.then((users) => {
  ...
  public getUser(id: number): Promise<User> {
     ...
     let user =  getEntityManager().getRepository(User).findOneById({id: id});
  ...
  public updateUser(user: User): Promise<User> {
   ...
   let userT = userRepository.findOne({id: user.id});

   let returnType = userT.then((u) => {
       u.email  = user.email;
       u.namel = user.namel;
  ...
  let userRepository = getEntityManager().getRepository(User);
   let userT = userRepository.findOne({id: id});
    userT.then((u) => {
